...
function ierg4210_prod_redirect_cat1() {
	// DB manipulation
		echo 'successfully redirected!';
		header('Location: adc.html');
		exit();
}

function ierg4210_redirect_cat() {
	header('Location: adc.html');
	return 1;
}

function ierg4210_prod_fetchcat1() {
	// DB manipulation
	global $db;
	$db = ierg4210_DB();
...
function ierg4210_prod_fetchcat2() {
	// DB manipulation
	global $db;
	$db = ierg4210_DB();
	$q = $db->prepare("SELECT * FROM products WHERE catid=2;");
	if ($q->execute())
		return $q->fetchAll();
}
...
function ierg4210_prod_edit() {
	...
	global $db;
	$db = ierg4210_DB();
	$q = $db->prepare("UPDATE products SET name =?, catid=?, price=?, description=? WHERE pid=?");
	$q->execute(array($_POST['name'],$_POST['catid'],$_POST['price'], $_POST['description'],$_POST['pid']));

	echo 'successfully uploaded!';
	header('Location: admin.html');  //redirecting needs changing
	exit();
}
