...
function ierg4210_trans_fetchall() {
	// DB manipulation
	global $db;
	$db = ierg4210_DB();
	$q = $db->prepare("SELECT * FROM orders ORDER by oid DESC LIMIT 10;");
	if ($q->execute())
		return $q->fetchAll();
}

function ierg4210_pru_fetchall() {
	// DB manipulation
	global $db2;
	$db2 = order_DB();
	$q = $db2->prepare("SELECT * FROM purchased_list ORDER by purid DESC LIMIT 15;");
	if ($q->execute())
		return $q->fetchAll();
}
...
function ierg4210_prod_insert() {
	...
	if ($_FILES["file"]["error"] == 0
		...
		if (move_uploaded_file($_FILES["file"]["tmp_name"], "/var/www/html/phases/incl/img/" . $lastId . ".jpg")) {
...
function ierg4210_prod_edit() {
	...
	if ($_FILES["file"]["error"] == 0
		...
		//$lastId = $db->lastInsertId();
		// Note: Take care of the permission of destination folder (hints: current user is apache)
		if (move_uploaded_file($_FILES["file"]["tmp_name"], "/var/www/html/phases/incl/img/" . $_POST['pid'] . ".jpg")) {
		// redirect back to original page; you may comment it during debug
